functions {
  ...
  real compute_V(real y, real p, real x1, real x2, real gamma) {
    real w = pip(p, gamma);
    real gain = w * (x1 - y);
    real loss = (1 - w) * (y - x2);
    return gain - loss;
...
model{
  ...
  for(i in 1:N){
    ...
    for(j in 1:5){
      ...
      V_c[j] = -abs(beta * compute_V(Y_c, p[i], x1[i], x2[i], gamma));
    }
    ...
    for(j in 1:5){
      ...
      V_f[j] = -abs(beta * compute_V(Y_f, p[i], x1[i], x2[i], gamma));
    }
