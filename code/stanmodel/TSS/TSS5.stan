functions {
  ...
  real compute_V(real y, real p, real x1, real x2, real gamma, real alpha, real lambda) {
    real w = pip(p, gamma);
    real gain = w * pow(x1 - y, alpha);
    real loss = (1 - w) * pow(y - x2, alpha);
...
model{
  ...
  for(i in 1:N){
    ...
    for(j in 1:5){
      ...
      V_c[j] = -abs(1 * compute_V(Y_c, p[i], x1[i], x2[i], gamma, alpha, lambda));
    }
    ...
    for(j in 1:5){
      ...
      V_f[j] = -abs(1 * compute_V(Y_f, p[i], x1[i], x2[i], gamma, alpha, lambda));
    }
