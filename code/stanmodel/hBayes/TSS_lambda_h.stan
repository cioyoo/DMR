functions {
  vector compute_V(vector Y, real p, real x1, real x2, real gamma, real alpha, real lambda) {
    real w = inv_logit(gamma*logit(p));
    return w * pow(x1-Y, alpha) - lambda * (1-w) * pow(Y-x2, alpha);
  }
}
...
transformed parameters {
  ...
  for(i in 1:N){
    gamma[i] = exp(mu_p[1]+sigma[1]*gamma_pr[i]);
    alpha[i] = exp(mu_p[2]+sigma[2]*alpha_pr[i]);
    lambda[i] = exp(mu_p[3]+sigma[3]*lambda_pr[i]);
  }
}
...
model{
  ...
  for(i in 1:N){
    for(j in 1:Tsubj[i]){
      // Coarse selection
      vector[5] V_c = -fabs(compute_V(Y_c[i,j], p[i,j], x1[i,j], x2[i,j], gamma[i], alpha[i], lambda[i]));
      int action_c = (c_idx[i,j]) / 5 + 1;
      action_c ~ categorical_logit(V_c);
      // Fine selection
      vector[5] V_f = -fabs(compute_V(Y_f[i,j], p[i,j], x1[i,j], x2[i,j], gamma[i], alpha[i], lambda[i]));
...
generated quantities {
  ...
  real mu_alpha = exp(mu_p[2]);
  real mu_lambda = exp(mu_p[3]);
  matrix[N, T] log_lik;
