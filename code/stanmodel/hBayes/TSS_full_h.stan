functions {
  vector compute_V(vector Y, real p, real x1, real x2, real gamma, real alpha_gain, real alpha_loss, real lambda) {
    real w = inv_logit(gamma*logit(p));
    return w * pow(x1-Y, alpha_gain) - lambda * (1-w) * pow(Y-x2, alpha_loss);
  }
}
...
transformed parameters {
  ...
  vector<lower=0>[N] lambda = exp(mu_p[4]+sigma[4]*lambda_pr);
}
...
model{
  ...
  for(i in 1:N){
    for(j in 1:Tsubj[i]){
      // Coarse selection
      vector[5] V_c = -fabs(compute_V(Y_c[i,j], p[i,j], x1[i,j], x2[i,j], gamma[i], alpha_gain[i], alpha_loss[i], lambda[i]));
      int action_c = (c_idx[i,j]) / 5 + 1;
      action_c ~ categorical_logit(V_c);
      // Fine selection
      vector[5] V_f = -fabs(compute_V(Y_f[i,j], p[i,j], x1[i,j], x2[i,j], gamma[i], alpha_gain[i], alpha_loss[i], lambda[i]));
